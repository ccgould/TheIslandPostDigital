...
namespace IslandPostAdministrator.Controls;
public class PageControlHelper : Control
{
    public static readonly DependencyProperty ShowProperty = DependencyProperty.RegisterAttached(
        "Show",
        typeof(bool),
        typeof(PageControlHelper),
    ...
    public PageControlHelper()
    {
        Loaded += static (sender, _) => ((PageControlHelper)sender).OnLoaded();
        Unloaded += static (sender, _) => ((PageControlHelper)sender).OnUnloaded();
    ...
    {
        Wpf.Ui.Appearance.ApplicationTheme currentTheme = Wpf.Ui.Appearance.ApplicationThemeManager.GetAppTheme();
